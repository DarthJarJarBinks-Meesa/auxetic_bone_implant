...
def _build_mesh_reference_block(
    ...
    if mesh_inp_fragment.exists():
        ...
        block = textwrap.dedent(f"""\
            ...
            **             *NSET (ALL_NODES, FIXED_FACE, LOADED_FACE,
            **             LEFT_SUPPORT, RIGHT_SUPPORT), and *ELSET (ALL_ELEMS)
            **             blocks matching the gmsh mesh.
            **
            ...
            **   LOADED_FACE  — nodes on the load-application face
        ...
        """)
        warnings.append(
            f"No pre-converted mesh include file found at "
            f"'{mesh_inp_fragment}'.  A MESH PLACEHOLDER block has been "
            f"written.  The input deck is structurally complete but will not "
            f"run in CalculiX until the mesh include is provided.  See the "
            f"comments in the file for instructions."
        )
...
def _build_load_block(loadcase: LoadCaseRecord) -> str:
    ...
    lines = [
        "**",
        "** === APPLIED LOADS ===",
        "** NOTE: LOADED_FACE is a symbolic node set placeholder.",
        "*CLOAD",
    ]
    ...
    if lc_type == LoadCaseType.AXIAL_COMPRESSION:
        ...
        lines += [
            f"** Three-point bending: {abs(force):.2f} N applied in -Z direction at LOADED_FACE",
            "** Support reactions computed by CalculiX at LEFT_SUPPORT, RIGHT_SUPPORT.",
            f"LOADED_FACE, 3, {force:.4f}",
        ]
